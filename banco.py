        ...
        if USE_POSTGRES:
            ...
            cur.execute(f"""
                SELECT e.id, e.doc_id, e.doc_nome,
                       e.funcionario_id,
                       f.nome AS funcionario, f.cargo, f.celular, f.cpf, f.admissao,
...
def buscar_modelo(doc_id: str, cargo: str = None) -> bytes | None:
    """
    Retorna bytes do modelo .docx mais recente.
    Tenta cargo específico primeiro, depois fallback para modelo geral.
    """
    conn = conectar()
    try:
        if USE_POSTGRES:
